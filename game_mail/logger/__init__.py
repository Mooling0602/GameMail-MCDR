    ...
    def log(self, level, message: str, module: Optional[str]=None):
        if self.levels[level] >= self.levels[self.level]:
            ...
            if module is not None:
                if cfg.logger_config["time"]:
                    print(f"[GameMail - {module}] [{timestamp} {colored_level}] {message}")
                else:
                    print(f"[GameMail - {module}/{colored_level}] {message}")
            else:
                if cfg.logger_config["time"]:
                    print(f"[GameMail - {timestamp} {colored_level}] {message}")
                else:
                    print(f"[GameMail - {colored_level}] {message}")
